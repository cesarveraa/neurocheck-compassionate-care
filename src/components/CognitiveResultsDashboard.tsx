...
import { BarChart, CartesianGrid, XAxis, YAxis, Bar, ResponsiveContainer, LineChart, Line } from "recharts";
        ...
        <Card>
          ...
          <CardContent>
              ...
              <div className="relative h-32 w-32">
                ...
                <svg className="h-full w-full" viewBox="0 0 100 100">
                  ...
                  <circle 
                    ...
                    r="45" 
                    fill="transparent" 
                    stroke={getRiskColor(results.overallScore)} 
                    strokeWidth="10" 
                    strokeDasharray={`${2 * Math.PI * 45 * results.overallScore / 100} ${2 * Math.PI * 45 * (100 - results.overallScore) / 100}`}
                    strokeLinecap="round"
            ...
            <p className="text-center mt-2">
              Nivel de riesgo: <span className={`font-medium ${getRiskColor(results.overallScore).replace('bg-', 'text-')}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-[150px]">
              ...
              >
                <LineChart data={results.timeData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            ...
            <p className="text-center mt-2">Tiempo promedio: {results.responseTime}s</p>
          </CardContent>
        </Card>
                ...
                <div key={index} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${domain.status === "green" ? "bg-neuro-green" : domain.status === "yellow" ? "bg-neuro-yellow" : "bg-neuro-red"}`}
        ...
        <CardContent>
          <div className="h-[250px]">
            <ChartContainer config={config} className="h-full w-full">
              <BarChart data={results.domains} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.1)" />
                ...
                <Bar
                  ...
                  maxBarSize={50}
                >
                  {results.domains.map((entry, index) => (
                    <rect
                      key={index}
                      fill={entry.status === "green" ? "#06D6A0" : entry.status === "yellow" ? "#FFD166" : "#FF6B6B"}
                    />
                  ))}
                </Bar>
              </BarChart>
